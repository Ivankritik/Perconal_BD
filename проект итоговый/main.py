...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.add_img = tk.PhotoImage(file='./img1/add.png')
        ...
        self.update_img = tk.PhotoImage(file='./img1/update.png')
        ...
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email', 'price'),
    ...
    def update_record(self, name, tel, email, price):
        self.db.c.execute('''UPDATE db SET name=?, tel=?, email=?, price=? WHERE ID=?''',
                          (name, tel, email, price, self.tree.set(self.tree.selection()[0], '#1')))
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        label_price = tk.Label(self, text='Зарплата')
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, price text)''')
        # сохранение изменений БД
        self.conn.commit()

    # метод добавления в БД
    def insert_data(self, name, tel, email, price):
        self.c.execute('''INSERT INTO db (name, tel, email, price) VALUES (?, ?, ?, ?)''',
                       (name, tel, email, price))
        self.conn.commit()
